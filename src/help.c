...
static void help_info(void)
{
  fprintf(stdout,
    ...
    "      compressed, or mixed).\n"
    "    - Per-locus summary table: index, sequence count, and length per\n"
    "      locus. For pattern-compressed input two extra columns appear:\n"
    "      `model` (JC69 or GTR) and `weights_sum` (the original site\n"
    "      count represented by that locus). When the input has more than\n"
    "      10 loci the table is truncated to first 5 + last 5 with an\n"
    "      omission marker; pass --per-locus to show every row.\n"
    "    - Aggregate section: total alignments, total sequences, unique\n"
    "      sequence labels, per-locus length statistics (mean / min /\n"
    "      max), and the total site or pattern counts; when compressed,\n"
    "      a compression ratio is also reported.\n"
    "\n"
